...
pub struct ControlPlane {
    ...
    ///
    /// Multi-session (W2-C): the hub projects a single **focused** session
    /// (`active_session_id`). Background live sessions keep running; their
    /// post-persist updates only advance the projection when they are focused
    /// ([`PresentationHub::publish_session_update`]). Switch focus via
    /// [`ControlPlane::presentation_focus`].
