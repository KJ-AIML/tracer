...
|---|---|
| **Gate 2.2.3** | **PASS** |
| Doctor (authoring host) | **READY** - drivers present; Edge/msedgedriver 150.0.4078.65 exact match |
| L2 packaged app smoke | **PASS** |
| L3-I WebView driver infra | **PASS** |
| L3-J full GUI product journey | **PASS** - GJ-01..12 all PASS, twice (repeatability) |
| Network / live Grok credentials | **Not used** - fake ACP + temp SQLite only |
...
| Wave 2.3 | **Not started** - entry criteria only |
...
| Task claim | write / host `grok-build` / session `heli-ses-22f90a54-f5c7-4e71-b0b2-5db97cf84172` |
...
| `e988037` | `docs(w2.2-b): L3-J architecture, journey spec, matrix, results` |
| `29277b5` / `87d2331` | docs pin commits |
...
| `9aab1a8ec9b2838bc27be8008d49f3c59094f63d` | `docs(w2.2.3): pin integration docs commit SHA` |
...
|---|---|---|
| `withGlobalTauri: true` | Product IPC surface for real invoke | **No** - required for Tauri mode honesty |
| `data-testid="tracer-*"` + ready marker | Stable automation + a11y status | **No** - additive |
| Path-based project register | Product form (native picker optional) | **No** |
| Fake ACP scenario select on session create | Product scenario catalog (fake-only world) | **No** - not live auto-approve |
| Non-blocking `submitPrompt` soft-poll | Concurrent approval/cancel UX | **No** - product improvement |
| Presentation focus on create/open | Multi-session contract | **No** |
| Backend badge always visible | Fail-closed honesty | **No** - strengthens product |
| `__TRACER_E2E__` skip `window.confirm` | Harness-only flag injected by WebDriver | **No** - product keeps confirm |
...
| GJ-07 two-session focus | Create A/B + Open via list | - |
...
| GJ-11 fail-closed | Invalid register path via form | - |
| GJ-12 clean shutdown | Soft stop + harness orphan verify | - |
...
1. Edge auto-update can break `major(msedgedriver)==major(Edge)` - re-run opt-in setup apply.
2. Serial GUI suite is host- and timing-sensitive; use failure artifacts under `artifacts/tauri-e2e/`.
3. Fake ACP scenarios are the only approval/crash source in L3-J - not live-provider parity.
...
| Item | Value |
|---|---|
| Integration branch tip (docs body) | `0cada17213b58a4352e1fca9008dad4d9479f463` |
| Integration pin commits | `9aab1a8` / final FF tip set after main advance |
