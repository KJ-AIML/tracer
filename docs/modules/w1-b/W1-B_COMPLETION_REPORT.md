...
| Fixture | Covers protocol section 10 corpus item |
|---|---|
| `happy_prompt_stream.json` | Happy-path stream |
| `tool_with_approval.json` | Tool + approval request/resolve |
| `unknown_vendor_notification.json` | Unknown notification to `adapter.protocol.unknown` |
| `protocol_error.json` | Malformed to `adapter.protocol.error` |
...
See `SHARED_MANIFEST_REQUESTS.md`:
...
| `87b89277db64d28492ee1bda41c0fd122c0b6aa5` | `docs(w1-b): record completion report commit SHA` |

Tip after this amend/fix: see latest `git log -1` on branch (may add a clean-encoding fix commit).
